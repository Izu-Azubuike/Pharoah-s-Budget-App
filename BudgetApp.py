...
# Alaba Olanrewaju contributed indirectly to this.
...
def debit():
    ...
    pick = int(input('\nPress (1) To continue with your transaction\nPress (2) To stop transaction\n'))
    if (pick == 1):
        user = input("\n**** Select one of the budget(s) mentioned above ****\n")
        if user in database:
            print('Note: You must leave at least $1 must in your budget.')
...
def credit():
    ...
    if (pick == 1):
        ...
        print('\nYou terminated the transaction')
        menu()
    else:
        print('\nInvalid selection')
        deposit()
...
def out():
    ...
    if (pick == 1):
        print("\nThank you for budgeting with us, do have a nice day!")
        quit()
    elif (pick == 2):
        menu()
    else:
